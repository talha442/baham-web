...
class VehicleContractTest(TestCase):
    def setUp(self):
        ...
        self.superuser = User.objects.create_superuser(
            username='admin', email='admin@dareecha.com', password='admin')
        self.owner = User.objects.create_user(
            username='user', email='a@gmail.com', password='123')
        self.companion = UserProfile.objects.create(
            user=self.owner, birthdate=date(2000, 2, 3), user_id=1)
        self.model = VehicleModel.objects.create(
            vendor='KIET', model='Unicorn-1', type=VehicleType.VAN, capacity=3)
    ...
    def test_one_vehicle_per_owner(self):
        ...
        self.assertEqual(Vehicle.objects.filter(
            owner=self.owner).count(), 1, 'No Owner can own 2 Vehicles')
    # Test Case #2
